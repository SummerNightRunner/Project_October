...
`PUT /users/{user_id}/ratings/{movie_id}`. Endpoints пользовательской истории и
оценок защищены API-key auth.
...
- `403`: ключ валиден, но не содержит scope, нужный endpoint.
...
Статус: минимальная реализация API-004 с API-key auth из API-005.
...
- `401`: API key отсутствует, неверен, отозван, истек или связан с неактивным
  API client;
- `403`: API key не содержит `history:read`;
...
- `401`: API key отсутствует, неверен, отозван, истек или связан с неактивным
  API client;
- `403`: API key не содержит `history:write`;
...
- `401`: API key отсутствует, неверен, отозван, истек или связан с неактивным
  API client;
- `403`: API key не содержит `ratings:write`;
- `404`: `movie_id` отсутствует в `movie_catalog_entries`;
...
проверке обновляет `last_used_at`.
